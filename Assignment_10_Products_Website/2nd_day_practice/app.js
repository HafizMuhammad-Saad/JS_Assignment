var watches = {
    ROLEX: {
      ...
      RolexOyster: {
        ...
        fullName: "Rolex Oyster Perpetual Datejust 36 Champagne Index - 126231-62801",
        category: "Men's Watch",
        color: [, "golden", "silver"],
    ...
    AUDEMARSPIGUET: {
      TamaraRalph: {
        ...
        fullName:"AUDEMARS PIGUET : Royal Oak Concept Flying Tourbillon “Tamara Ralph” Limited Edition",
        category: "Women's Watch",
        color: "golden",
...
//  console.log(watches['ROLEX']['RolexDateJust28']['price']);
...
`
 for (var brands in watches) {
    for (var products in watches[brands]) {
        var details = watches[brands][products];
        data += `
            <div class="col ">
              <div class="card h-100 rounded">
                <img src="${details.img}" class="card-img-top" alt="..." width="100">
                <div class="card-body bg-light">
                  <h5 class="card-title">${details.name}</h5>
                  <h6 class="card-text text-danger">${details.price}</h6>
                  <p class="card-text text-success">${details.category}</p>
                  <button id="details" class="card-text text-info btn" onclick="showDetails()">See Details...</button>
...
function displayProduct() {
    for (var key in watches[selectedBrand]) {
        var detail = watches[selectedBrand][key];
        data = `
            <div class="card  style="width: 18rem;" rounded">
                <img src="${detail.img}" class="card-img-top" alt="..." >
                <div class="card-body bg-light">
                  <h5 class="card-title">${detail.name}</h5>
                  <h6 class="card-text text-danger">${detail.price}</h6>
                  <p class="card-text text-success">${detail.category}</p>
                  <button id="details" class="card-text text-info btn" onclick="showDetails()">See Details...</button>
